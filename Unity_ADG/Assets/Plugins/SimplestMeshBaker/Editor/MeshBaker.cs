...
namespace SimplestMeshBaker
{
    public class MeshBaker : EditorWindow
    {
        ...
        private static bool convertToAsset;

        [MenuItem("GameObject/MeshBaker/Create Mesh Asset",false,2)]
        private static void CreateMeshAsset()
        {
            ...
            if (meshFilter)
            {
                try
                {
                    AssetDatabase.CreateAsset(meshFilter.sharedMesh, "Assets/NewBakedMesh.mesh");
        ...
        [MenuItem("GameObject/MeshBaker/Bake Meshes", false, 0)]
        private static void BakeMeshes(MenuCommand menuCommand)
        {
            ...
            convertToAsset = EditorUtility.DisplayDialog("Simplest Mesh Baker", "Do you want to create Asset?", "Yes", "No");
            ...
            List<Vector2> uvs, List<int> triangles, Material material)
        {
            ...
            if (convertToAsset)
            {
                AssetDatabase.CreateAsset(newMesh, "Assets/NewBakedMesh.mesh");
                AssetDatabase.SaveAssets();
            }
        }
